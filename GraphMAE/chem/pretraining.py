...
from aim import Run
from src.utils.aim_callback import get_experiment_folder_aim
from src.utils.onnx_export import save_model_onnx

aim_run = Run(experiment="GraphMAE_authors")
...
def get_input_graph_viz(model, dec_pred_atoms, device):
    class GraphMAE(torch.nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.encoder = model
            self.decoder = dec_pred_atoms

        def forward(self, x, edge_index, edge_attr, masked_atom_indices):
...
def train_mae(args, model_list, loader, optimizer_list, device, alpha_l=1.0, loss_fn="sce"):
    ...
    for step, batch in enumerate(epoch_iter):
        ...
        # params.update(dec_pred_atoms.named_parameters())
        # save_model_graph(loss, params, "debug")
        loss.backward()
        ...
        aim_run.track(loss.item(), name="train_loss", context={'subset': 'train'})
...
def main():
    ...
    graph_model = get_input_graph_viz(model, atom_pred_decoder, device)
    list_names = ["x", "edge_index", "edge_attr", "masked_atom_indices"]
    batch = next(iter(loader))
    batch = batch.to(device)
    input_values = tuple([batch.to_dict()[k] for k in list_names])
    save_model_onnx(graph_model, input_values, list_names, ["node_predictions"], get_experiment_folder_aim(aim_run))
